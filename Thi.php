<?php
    ...
    if (isset($_POST['btn']) || isset($_POST['auto_submit'])) {
        ...
        header("Location: XemDiemAd.php?diem=$diem&tong=$tong_so_cau_hoi&this_id=$this_id");
        exit();
    }
    if (isset($_POST['btnq'])) {
        header('Location: VaoThiAd.php');
    }
    include "dau.php";
?>
...
<script type="text/javascript">
    ...
    function startTimer(minutes, seconds) {
        ...
        var interval = setInterval(function () {
            ...
            document.querySelector('#thoi_gian').innerHTML = '<i class="bi bi-clock"></i> ' + minutes + ' : ' + seconds;
            if (--totalSeconds <= 0) {
        ...
        <form id="thi" method="POST">
            ...
            <?php
                if ($thoi_gian_hien_tai < $thoi_gian_bat_dau) {
                    echo "<h2>Chưa đến thời gian bắt đầu bài thi!</h2><br>";
                    echo '<button type="submit" class="btn btn-primary" name="btnq">Quay lại</button>';
                    exit();
                }
                if ($thoi_gian_hien_tai > $thoi_gian_ket_thuc) {
                    echo "<h2>Thời gian làm bài đã hết!</h2><br>";
                    echo '<button type="submit" class="btn btn-primary" name="btnq">Quay lại</button>';
                    exit();
                }
            ?>
